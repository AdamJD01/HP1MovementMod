...
class UImpactSoundSet;
class USoundContainer;

#define MP2_SKIP_BUFFER_SIZE 0x8000
...
class ENGINE_API UAudioSubsystem : public USubsystem
{
	...
	virtual void Restore( UViewport* Viewport )=0;
	...
	virtual UBOOL PlaySound( AActor* Actor, INT Id, USound* Sound, FVector Location, FLOAT Volume, FLOAT Radius, FLOAT Pitch, FLOAT, FLOAT, INT Flags, FLOAT FadeDuration, FLOAT Priority = 0.f )=0;
	virtual UBOOL StopSound( AActor* Actor, USound* Sound, FLOAT FadeOutTime )=0;
	virtual UBOOL StopSound( AActor* Actor, USound* Sound )=0;
	virtual INT PlayMusic( FString Song, FLOAT FadeInTime, INT )=0;
	...
	virtual UBOOL ModifySound( AActor* Actor, INT Id, USound* Sound, BYTE, FLOAT );
...
public:
	...
	INT Length() const;
	INT EndPos() const;
	INT UncompressedLength() const;
	void SetIfStereo();
	void SetupVarsForMP2();
	INT Buffer( void *, INT, INT ) const;

private:
	// KWG.
	SetupFromFilename( const TCHAR* Filename );
};

template<class T> class SetArray;
...
public:
	...
	SoundStream( const USound* Sound );
	const USound* PSound();

	virtual void _UnknownVirtual0()=0;
	virtual void _UnknownVirtual1()=0;
	virtual void _UnknownVirtual2()=0;
...
class ENGINE_API USound : public UObject
{
	...
	// Constructor.
	USound();
	USound( UMusic* Music, bool bLooping ); // KWG.
	...
	// KWG.
	bool IsMusic() const;
	bool IsStreamingFromFile() const;
	void SetupLoopBuffer();
	void SetupVarsForVersion70();
	void SetupWAVheader( SetArray<BYTE>& );
	LocalPtr<SoundStream> newSoundStream();
	virtual void SetupFromFilename( const TCHAR* Filename );

private:
	// KWG.
	void CalculateUncompressedLength();
};
