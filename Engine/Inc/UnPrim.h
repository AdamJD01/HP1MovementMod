...
class ENGINE_API UPrimitive : public UObject
{
	...
	virtual FBox GetCollisionBoundingBox( const AActor* Owner, bool ) const;
	virtual FVector GetCollisionExtent( const AActor* Owner ) const;
	virtual void DrawCollisionBounds( AActor* Owner, class URenderBase* Render, struct FSceneNode* Frame, FPlane, DWORD );
	virtual void ValidateActor( const AActor* Owner );
};
