...
public:
	INT Num() const
	{
		guardSlow(TRefArray::Num);
		return ArrayNum;
		unguardSlow;
	}
	T& operator()( INT i )
	{
		guardSlow(TRefArray::operator());
		checkSlow(i>=0);
	...
	{
		guardSlow(TRefArray::operator()const);
		checkSlow(i>=0);
		checkSlow(i<=ArrayNum);
		checkSlow(Data);
		return ((T*)Data)[i];
...
struct ENGINE_API FAnimVec
{
	...
	// Constructors.
	FAnimVec(void);
	FAnimVec( const FVector& V, FLOAT F );
	FAnimVec( const FQuat& Q );

	// FAnimVec interface.
	FVector Vector( FLOAT V ) const;
	FQuat Quat() const;

protected:
	void Init( const FVector& V, FLOAT F );
	static INT IntVal( FLOAT F );
};
...
struct VJointPos
{
	...
	{
		return Ar << V.Place.Orientation << V.Place.Position << V.Length << V.XSize << V.XSize << V.ZSize;
	}
};
...
struct AnalogTrackDigestInfo
{
	TArray <FQuat>   KeyQuat;   // Orientation key track
	TArray <FVector> KeyPos;    // Position key track
	TArray <INT>     KeyTimeIndex;  // For each key, time when next key takes effect (measured from start of track.)
};
...
{
	DWORD Flags;       // reserved 
	TRefArray<FAnimVec> KeyQuat;   // Orientation key tr
	TRefArray<FAnimVec> KeyPos;    // Position key track
	TRefArray<BYTE>     KeyTime;  // For each key, time when next key takes effect (measured from start of track.)
	FLOAT KeyPosScale;
	FLOAT KeyTimeScale;
	friend FArchive &operator<<( FArchive& Ar, AnalogTrack& A )
	{
		return Ar << A.Flags << A.KeyQuat << A.KeyPos << A.KeyTime << A.KeyPosScale << A.KeyTimeScale;
	}
};
...
class ENGINE_API USkeletalMesh : public ULodMesh
{
	...
	// KWG.
	FBox MeanBoundingBox; // Arithmetic mean of BoundingBoxes.
	...
	INT BoneIndex( FName Name ) const;
...
private:
	void ApplyAnim( AActor* Owner, CFSkelHeader* Header, bool ) const;
...
class ENGINE_API UAnimation : public UObject
{
	...
	virtual INT MemFootprint()
	{
		...
		INT TotalMem = 0;
		TotalMem += sizeof(UAnimation);
		TotalMem += sizeof(FNamedBone)*RefBones.Num();
		TotalMem += sizeof(FAnimVec)*KeyQuats.Num();
		TotalMem += sizeof(FAnimVec)*KeyPoses.Num();
		TotalMem += sizeof(BYTE)*KeyTimes.Num();
		TotalMem += sizeof(MotionChunk)*Moves.Num();
		for(INT i=0;i<Moves.Num();i++)
		{
			TotalMem += sizeof(AnalogTrack)*Moves(i).AnimTracks.Num();
		}
		TotalMem += sizeof(FMeshAnimSeq)*AnimSeqs.Num();
		return TotalMem;
		unguard;
	}
	...
	// KWG.
	TArray<FAnimVec>      KeyQuats;
	TArray<FAnimVec>      KeyPoses;
	TArray<BYTE>          KeyTimes;
