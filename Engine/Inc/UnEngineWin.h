...
#if 1
		// Remap unlimited tickrate to the maximum safe tickrate.
		if ( MaxTickRate<=KINDA_SMALL_NUMBER )
			MaxTickRate = 200.0;

		// Match Clamp in ULevel::Tick().
		MaxTickRate = Clamp( MaxTickRate, 10.0, 200.0 );

		// Delta<1.0 is to make sure game won't hang for too long in case time overflows.
		DOUBLE Delta;
		while ( ((Delta=(1.0/MaxTickRate)-(appSeconds()-OldTime))>0.0) && Delta<1.0 )
		{
			// Just sleep if we have enough time left. This is to avoid appSleep() returning much later then desired.
			if ( Delta>=0.0015 )
				appSleep(0.0010);
		}
#else //oldver
