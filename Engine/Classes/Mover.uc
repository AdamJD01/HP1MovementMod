...
var(MoverSounds) sound      FailSound;        // Optional sound when no movement.
...
{
	local bool bSaveCollide;

	bSaveCollide = bCollideWorld;
	bCollideWorld = false;
	OldPos = BasePos;
	SetLocation(BasePos);
	bCollideWorld = bSaveCollide;
}
...
function PostBeginPlay()
{
	...
	// @PAB we need to do this here for the moment for defaults
	// This will need to be moved later
...
function bool IsRelevant( actor Other )
{
	if( bProjTarget )
	{
		if( Projectile(Other) != none )
			return Projectile(Other).IsRelevantToMover();
		else
			return false;
	}
	else if( BumpType == BT_PawnBump )
		return Other.bIsPawn && Other.Mass >= 10;
	else if( BumpType == BT_PlayerBump )
