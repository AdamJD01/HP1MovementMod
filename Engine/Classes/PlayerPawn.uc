...
var plane DesiredFlashFog, ConstantGlowFog, InstantFog;
var float FadeRate;
...
var byte  PercentMusicVolume;
var bool bDontLoopSong;
var MusicEvent currentMusicEvent;

var const bool bSongFinished; // set internally to indicate when song finishes
...
var() globalconfig bool 	bAutoJump;		// Jump off ledges when advantageous.
...
var globalconfig enum EDifficulty
{
	DifficultyEasy,
	DifficultyMedium,
	DifficultyHard,
...
var globalconfig enum EObjectDetail
{
...
// Same as strafe aStrafe, but always gets set
var float aSideMove;
...
exec function SShot()
{
	...
	ConsoleCommand("set ini:Engine.Engine.ViewportManager Brightness 0.65");
...
{
	DesiredFlashFog = PlaneV( 0.001 * fog, scale );
}

function ClientInstantFlash( float scale, vector fog )
{
	InstantFog = PlaneV( 0.001 * fog, scale );
}
...
{
	ConstantGlowFog += PlaneV( 0.001 * fog, scale );
}

function ClientFadeIn( vector color, float time )
{
	FlashFog = PlaneV( 0.001 * color, 0 );
	ConstantGlowFog = PlaneV( 0.001 * color, -1 );
	FadeRate = 1.0/Time;
}

function ClientFadeOut( vector color, float time )
{
	FlashFog = PlaneV( 0.001 * color, 1 );
	ConstantGlowFog = PlaneV( 0.001 * color, 0 );
	FadeRate = -1.0/time;
}
...
function ClientSetMusic( music NewSong, byte NewSection, byte NewCdTrack, EMusicTransition NewTransition, optional bool bDontLoop )
{
	Song        = NewSong;
	SongSection = NewSection;
	CdTrack     = NewCdTrack;
	Transition  = NewTransition;
	bDontLoopSong = bDontLoop;
}

function ClientSetPercentMusicVolume (byte newPercentMusicVolume)
{
	PercentMusicVolume = newPercentMusicVolume;
}
...
//************************************************************************
	...
	{
		// Strafe.
// @PAB we do not want to do this with HP
...
function Actor FacingActor( float radius, optional vector extent )
{
	local vector start, end;
	local vector dummy;

	start = Location + vec(0,0,EyeHeight);
	end = start + (vec(radius,0,0) >> ViewRotation);
	return Trace( dummy, dummy, end, start, [bTraceActors] true, extent );
}
...
event ViewFlash(float DeltaTime)
{
	...
	if( FadeRate != 0 )
	{
		ConstantGlowFog.W += FadeRate * delta;
		if( ConstantGlowFog.W <= -1 )
		{
			ConstantGlowFog.W = -1;
			FadeRate = 0;
		}
		else if( ConstantGlowFog.W >= 0 )
		{
			ConstantGlowFog.W = 0;
			FadeRate = 0;
		}
...
// Convert a camera-relative vector to world coords, using player's current view.
// S.X and S.Y are camera coords, -1 to 1 from top left.
// S.Z is the distance from the camera in world units to place the result.
function vector CameraToWorld( vector c )
{
	local float unproj;
	local vector w;
	local actor Cam;
	...
	// Unproject the point.
	unproj = c.Z * Tan(FovAngle * Pi/360.0);
	w.X = c.Z;
	w.Y = c.X * unproj;
	w.Z = c.Y * unproj;

	// Convert from camera to world space, using player's current view.
	CamLoc = Location;
	CamRot = ViewRotation;
	PlayerCalcView( Cam, CamLoc, CamRot );

	return (w >> CamRot) + CamLoc;
}

// Don't use following function, it's broken.
native(542) final function vector ScreenToWorld( vector S );
