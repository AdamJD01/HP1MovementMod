...
#define ACTORPOLY_VERTEXBUFFER_SIZE     256  // Reaches 7 at most (clipped triangles) - used for tiles, lines and points also.
...
class DLL_EXPORT UD3DRenderDevice : public URenderDevice
{
	...
	{
		guard(UD3DRenderDevice::DrawTriangles);
		...
		if ( Indices )
		{
			Stats.Polys += NumIndices/3;
			Direct3DDevice7->DrawIndexedPrimitiveVB( D3DPT_TRIANGLELIST, VertsTLWindow.VertexBuffer7, VertsTLWindow.Unlock(), NumPts, Indices, NumIndices, 0 );
		}
		else if ( NumIndices==NumPts )
		{
			Stats.Polys += NumPts-2;
			Direct3DDevice7->DrawPrimitiveVB( D3DPT_TRIANGLEFAN, VertsTLWindow.VertexBuffer7, VertsTLWindow.Unlock(), NumPts, 0 );
		}
		else
		{
			Stats.Polys += NumPts/3;
			Direct3DDevice7->DrawPrimitiveVB( D3DPT_TRIANGLELIST, VertsTLWindow.VertexBuffer7, VertsTLWindow.Unlock(), NumPts, 0 );
		}
	...
	INT MaxVertices()
	{
		...
		return ACTORPOLY_VERTEXBUFFER_SIZE;
		unguard;
	}
