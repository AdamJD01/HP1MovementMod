...
protected:
	void Realloc( INT ElementSize )
	{
		...
		//warnf( TEXT("Realloc: (Data=0x%08x,ArrayMax=%i,ElementSize=%i,ArrayMax*ElementSize=%i)"), Data, ArrayMax, ElementSize, ArrayMax*ElementSize );
		Data = appRealloc( Data, ArrayMax*ElementSize, TEXT("FArray") );
		if ( (ArrayMax*ElementSize>0) )
			check(Data); // Not sure whether i should keep this in, but better crash in place to get at least a glimpse of an idea before things went south.
		//unguardf(( TEXT("%i*%i"), ArrayMax, ElementSize )); // oldver. keep it for searching.
		unguardf(( TEXT("(ArrayMax=%i,ElementSize=%i)"), ArrayMax, ElementSize ));
	}
