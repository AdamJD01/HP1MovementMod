...
// Plane functions.
static function plane PlaneF(float x, float y, float z, float w)
{
	local plane p;
...
static function plane PlaneV(vector v, float w)
{
	local plane p;
	p.x = v.x;	p.y = v.y;	p.z = v.z;	p.w = w;
	return p;
}

static function plane PlaneC(float c)
{
	local plane p;
...
static final operator(20) plane + ( plane A, plane B )
{
	return PlaneF( A.X+B.X, A.Y+B.Y, A.Z+B.Z, A.W+B.W );
}
static final operator(34) plane += ( out plane A, plane B )
{
...
static final operator(16) plane * ( plane A, float f )
{
	return PlaneF( A.X*f, A.Y*f, A.Z*f, A.W*f );
}
static final operator(34) plane *= ( out plane A, float f )
{
